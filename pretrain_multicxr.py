...
from CXR_dataset_MultiCXR import MultiCXR_Dataset
# import main_run
...
def get_pretrain_df(args):
    
    df = pd.read_csv('data_preparation/multicxr.csv', index_col=0)
    df['weights'] = np.where(df['dataset'].isin(['snu', 'vindr']), 100, 1)
    samples = []
    for category in args.categories:
        sample = df[df[category]==1].sample(5000, weights='weights')
        samples.append(sample)
        
    df_pretrain = pd.concat(samples).drop_duplicates().reset_index(drop=True)
    return df_pretrain
...
def train_dino(args):
    ...
    df_pretrain = get_pretrain_df(args)

    dataset = MultiCXR_Dataset(
        df=df_pretrain,
    ...
    student = utils.MultiCropWrapper(
        ...
        cls_head=CLSHead(inter_dim, 256, len(args.categories)),
    )
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, bce_loss, data_loader,
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            invalid_indices = (labels==-1).nonzero()
            loss[invalid_indices[:,0], invalid_indices[:,1]] = 0
            loss = loss.mean()
