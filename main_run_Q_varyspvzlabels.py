...
from Q_attention_viz import save_attention_maps

import CXR_dataset_Q
...
if utils.is_main_process():
    wandb.login()
    run = wandb.init(
        project="distl-vary-spvz-labels"
    )
...
def get_args_parser():
    ...
    parser.add_argument("--data_path", default="data_preparation")
    ...
    parser.add_argument('--wandb_log_steps', default=50, type=int)
    ...
    parser.add_argument('--attn_map_save_dir', type=str, default='attention_maps_progression')
    parser.add_argument('--imgs_for_attn_map', type=tuple,
                        default=(
                            '/home/wonjun/data/mimic-cxr-jpg-resized512/files/p10/p10002131/s52823782/051b7911-cb00aec9-0b309188-89803662-303ec278.jpg', # Pleural EFfusion
                            '/home/wonjun/data/mimic-cxr-jpg-resized512/files/p10/p10000980/s58206436/54affd39-8bf24209-232bac8a-df6c277a-398ee8a5.jpg', # Cardiomegaly
                            '/home/wonjun/data/mimic-cxr-jpg-resized512/files/p10/p10012292/s53627836/e1cb08b8-ad3fcb2e-f23a3675-b9f0db2e-f96e379e.jpg', # Pneumonia
                            '/home/wonjun/data/mimic-cxr-jpg-resized512/files/p10/p10011938/s56362279/c6eed867-d6efb38a-438501f9-9d2506e9-a0c958f8.jpg', # Edema
                            '/home/wonjun/data/mimic-cxr-jpg-resized512/files/p10/p10064049/s56368658/70fd493e-bf0919e7-33cbbfe7-3185c257-58aa23f7.jpg', # Lung Lesion
                        ))
...
def train_dino(args):
    ...
    dataset = CXR_dataset_Q.MIMIC_Staggered(
    ...
    l_dataset = CXR_dataset_Q.MIMIC_Staggered(
        data_path=args.data_path,
        mimic_path=args.mimic_path,
        lesions=list(args.lesions),
        transforms=transform,
        ...
        if it % args.correct == 0:
            ...
            for _, (images, labels) in enumerate(metric_logger.log_every(l_data_loader, 10, header)):
                ...
                with torch.cuda.amp.autocast(fp16_scaler is not None):
                    ...
                    # MIMIC labels can be -1 (for ambiguous presence of lesion)
                    # These should be skipped!
                    # (this is why we set reduction='none' when defining loss=nn.BCEWithLogitsLoss() above)
                    invalid_indices = (torch.cat((labels,labels), dim=0)==-1).nonzero()
                    l_loss_ce[invalid_indices[:,0], invalid_indices[:,1]] = 0
                    l_loss_ce = l_loss_ce.mean()
                ...
                if SPVZ_STEP % args.wandb_log_steps == 0:
                    if utils.is_main_process():
                        wandb.log({"spvz_loss": l_loss_ce.item()}, step=TOTAL_STEPS)
        
        if DINO_STEP % args.wandb_log_steps == 0:
            if utils.is_main_process():
                wandb.log({"distl_dino_loss": loss_dino.item(), "distl_ce_loss": loss_ce.item()}, step=TOTAL_STEPS)
