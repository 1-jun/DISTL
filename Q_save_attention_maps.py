...
def parse_args():
    ...
    parser.add_argument('--output_folder', type=str, default='/media/wonjun/HDD8TB/DISTL-attention-maps/mimic-split-10-30-30-30')
...
def save_attention_map(model, row, output_folder):
    ...
    # if there are no predicted classes, just return.
...
def main(args):
    ...
    for i, row in tqdm(df.iterrows(), total=len(df), colour='yellow', desc=f'saving attention maps'):
        
        # Skip if .npy and .json are already present in output_dir
        save_dir = os.path.join(args.output_folder,
                                f"p{str(int(row['subject_id']))[:2]}",
                                f"p{str(int(row['subject_id']))}",
                                f"s{str(int(row['study_id']))}")
        json_path = os.path.join(save_dir, row['dicom_id']+'_best_head_indices.json')
        attn_path = os.path.join(save_dir, row['dicom_id']+'.npy')
        if os.path.isfile(json_path) and os.path.isfile(attn_path):
            continue
        else:
            save_attention_map(model, row, args.output_folder)
