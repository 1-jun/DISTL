...
def get_args_parser():
    ...
    parser.add_argument("--data_path", default="data_preparation/mimic_multilabel_all.csv")
    ...
    parser.add_argument('--lam', type=float, default=0.5,
                        help="Loss scaling")
    parser.add_argument("--correct", default=1000, type=int,
...
def train_dino(args):
    ...
    dataset = CXR_dataset_Q.MIMIC_Multilabel_All_Dataset(
    ...
    print(f"\nDataset loaded: there are {len(dataset)} images.\n")
    ...
    # We want to train only train the last self-attention layer
    # so we freeze all other self-attention layers
    for block in student.blocks[:-1]:
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, bce_loss, data_loader,
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            loss_ce = bce_loss(torch.hstack(student_cls), torch.cat((labels, labels), dim=0))
            ...
            loss_ce[invalid_indices[:,0], invalid_indices[:,1]] = 0
